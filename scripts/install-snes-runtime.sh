...
RES="$PROJECT_DIR/vendor/tg5040/res"
...
ssh "$DEVICE_HOST" "mkdir -p '$REMOTE_SYSTEM/cores' '$REMOTE_SYSTEM/paks/Emus/SFC.pak' '$REMOTE_RES' /mnt/SDCARD/Roms/SFC /mnt/SDCARD/Saves/SFC && if [ -f '$REMOTE_SYSTEM/bin/minarch.elf' ] && [ ! -f '$REMOTE_SYSTEM/bin/minarch.elf.pre-sfc' ]; then cp '$REMOTE_SYSTEM/bin/minarch.elf' '$REMOTE_SYSTEM/bin/minarch.elf.pre-sfc'; fi"
...
scp "$RES/sfc-brick-mask.png" "$RES/sfc-crt-3x.png" "$DEVICE_HOST:$REMOTE_RES/"
ssh "$DEVICE_HOST" "chmod +x '$REMOTE_SYSTEM/bin/minarch.elf' '$REMOTE_SYSTEM/paks/Emus/SFC.pak/launch.sh' && sha256sum '$REMOTE_SYSTEM/cores/snes9x2005_plus_libretro.so' '$REMOTE_SYSTEM/bin/minarch.elf' '$REMOTE_RES/sfc-brick-mask.png' '$REMOTE_RES/sfc-crt-3x.png'"
