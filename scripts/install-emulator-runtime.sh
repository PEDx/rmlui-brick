...
for required in \
    ...
    "$RUNTIME_DIR/res/sfc-brick-mask.png"; do
    if [ ! -f "$required" ]; then
...
ssh "$DEVICE_HOST" "mkdir -p \
    ...
    '$REMOTE_STAGE/paks/Emus/GBA.pak' '$REMOTE_STAGE/paks/Emus/SFC.pak' \
    '$REMOTE_SYSTEM/bin' '$REMOTE_SYSTEM/cores' '$REMOTE_RES' \
    '$REMOTE_SYSTEM/paks/Emus/GB.pak' '$REMOTE_SYSTEM/paks/Emus/GBC.pak' \
    '$REMOTE_SYSTEM/paks/Emus/GBA.pak' '$REMOTE_SYSTEM/paks/Emus/SFC.pak' \
    /mnt/SDCARD/Roms/GB /mnt/SDCARD/Roms/GBC /mnt/SDCARD/Roms/GBA /mnt/SDCARD/Roms/SFC \
    /mnt/SDCARD/Saves/GB /mnt/SDCARD/Saves/GBC /mnt/SDCARD/Saves/GBA /mnt/SDCARD/Saves/SFC"
...
for system in GB GBC GBA SFC; do
...
echo "GB/GBC/GBA/SFC runtime installed on $DEVICE_HOST"
